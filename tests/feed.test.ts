...
beforeAll(async () => {
  ...
  const [fantasy, adventure, scifi] = await Promise.all([
    ...
    db.subject.create({ data: { name: "Test Adventure", slug: "test-adventure" } }),
  ...
  // books
  const [bookLib, bookHigh, bookTieHigh, bookTieLow, bookZero] = await Promise.all([
    db.book.create({
      data: {
        ...
        author: "Author",
        ratingCount: 0,
        bookSubjects: { create: [{ subjectId: subjectFantasyId }, { subjectId: subjectAdventureId }] },
      },
    }),
    db.book.create({
      data: {
  ...
  await Promise.all([ghostApp.ready(), authApp.ready()]);
  ...
  if (libRes.statusCode !== 201) {
    throw new Error(`Setup POST /library failed: ${libRes.statusCode} — ${libRes.body}`);
  }
});
...
afterAll(async () => {
  ...
  const testBookIds = [bookLibraryId, bookHighId, bookTieHighId, bookTieLowId, bookZeroId];
  await db.bookSubject.deleteMany({ where: { bookId: { in: testBookIds } } });
  await db.book.deleteMany({ where: { id: { in: testBookIds } } });
  await db.subject.deleteMany({ where: { id: { in: [subjectFantasyId, subjectAdventureId, subjectSciFiId] } } });

  await Promise.all([ghostApp.close(), authApp.close()]);
  await db.$disconnect();
});
...
describe("GET /books/feed — cursor pagination (personalised path)", () => {
  test("returns nextCursor when more results exist", async () => {
    const res = await authApp.inject({ method: "GET", url: "/books/feed?limit=1" });
    expect(res.json().nextCursor).not.toBeNull();
  });

  test("second page returns the next book in ranked order", async () => {
    const first = await authApp.inject({ method: "GET", url: "/books/feed?limit=1" });
    const { data: page1, nextCursor } = first.json();

    const second = await authApp.inject({ method: "GET", url: `/books/feed?limit=1&cursor=${nextCursor}` });
  ...
  test("nextCursor is null on the last page", async () => {
    ...
    const res = await authApp.inject({ method: "GET", url: "/books/feed?limit=50" });
